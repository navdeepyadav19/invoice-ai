...
If the invoice already has a number, finalize returns it unchanged and never assigns a second one. Together with the required [`Idempotency-Key`](/errors#idempotency), this means a retry can't leave a gap in your series.

## Sending

`POST /invoices/{id}/send` (scope `invoices:send`, `Idempotency-Key` required, [10 per hour](/errors#rate-limits)) emails the invoice PDF and a link to view it online.

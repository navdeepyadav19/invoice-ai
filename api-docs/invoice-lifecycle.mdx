...
- The response is `{ "data": { "id", "invoice_number", "emailed": true, "public_url" } }`.
...
`PATCH /invoices/{id}` accepts the same fields as create: `customer`, `currency`, `collection_method`, `due_date`, `description`, `footer` and `items`. Fields you leave out keep their current values, with two exceptions:

<Warning>
  - `customer` is currently **required on every `PATCH`**, not just on create. Send the invoice's current customer id if you aren't changing it.
  - `items`, when present, **replaces every line** on the draft. To change one line, use `POST /invoice-items` and `DELETE /invoice-items/{id}`, which return the whole recalculated invoice.
</Warning>
...
For the same reason there's no `invoice.overdue` webhook. To find overdue invoices, list `status=open` and check each `status`. See the note in [Pagination](/pagination#things-to-know).

## History

`GET /invoices/{id}/events` returns the invoice's full history, newest first. Each event is `{ id, type, meta, created_at }`. Event types match the [webhook events](/webhooks#events): `invoice.created`, `invoice.updated`, `invoice.finalized`, `invoice.emailed`, `invoice.email_failed`, `invoice.viewed`, `invoice.downloaded`, `invoice.paid` and `invoice.voided`.
